...
function addObserver(){
    ...
    for (var i=0;i<item.length;i++){
        ...
        DocumentObserver = new MutationObserver( Check );
...
function Check(){
    AnimeName = `AAM-${document.querySelectorAll('li[class="active"]')[1].querySelector('div.an-info').querySelector('div').querySelector('a').textContent}`
    console.log(AnimeName);
    updata(AnimeName)
}

function updata(AnimeName){
   ...
   function ToQBDownload(URL,AnimeName){
        ...
        formData.append('tags', `anime,${AnimeName}`);
...
if (window.location.pathname != '/'){
    AnimeName = `AAM-${document.querySelectorAll('.bangumi-title')[0].textContent}`
    console.log(AnimeName);
    updata(AnimeName);
}else {
    addObserver();
